...
import dev.ai4j.deepseek4j.completion.CompletionChoice;
import dev.ai4j.deepseek4j.completion.CompletionResponse;
...
public class DeepSeekStreamingResponseBuilder {
    ...
    public void append(ChatCompletionResponse partialResponse) {
        ...
        if (delta.toolCalls() != null && !delta.toolCalls().isEmpty()) {
            ...
            DeepSeekStreamingResponseBuilder.ToolExecutionRequestBuilder toolExecutionRequestBuilder
                    = indexToToolExecutionRequestBuilder.computeIfAbsent(toolCall.index(), idx -> new DeepSeekStreamingResponseBuilder.ToolExecutionRequestBuilder());
    ...
    public Response<AiMessage> build() {
        ...
        Map<String, Object> attributes =
                Map.of("reasoningContent", reasoningContent);


        CustomAiMessage customAiMessage = new CustomAiMessage(text, attributes);
        ...
        if (!toolName.isEmpty()) {
            ...
            customAiMessage =
                    isNullOrBlank(text) ? new CustomAiMessage(singletonList(toolExecutionRequest), attributes)
                            : new CustomAiMessage(text, singletonList(toolExecutionRequest), attributes);
